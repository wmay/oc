!
        INTEGER VOTES(102*6000),DIMS,POLARITY(3),&
                EXITSTATUS
        DOUBLE PRECISION  CLASSIFY((102+6000)*4),&
                FITS(2),VOLUME(102),&
                IDEALPOINTS(102*25),&
                MIDPOINTS(6000),NORMALVECTORS(6000*25),&
                EIGENVALUES(102)
! 
 !     OPEN(1,FILE='DELEGATES2.ORD',STATUS='OLD')
!      OPEN(1,FILE='FORT.99',STATUS='OLD')
!      OPEN(1,FILE='S90_STRIPPED.ORD',STATUS='OLD')
!      OPEN(1,FILE='TESTIT.ORD',STATUS='OLD')
!
!  100 FORMAT(I5,2X,6000I1)
  100 FORMAT(3X,I5,28X,6000I1)
  200 FORMAT(I4,4F8.1,20F9.4)
  250 FORMAT(2F10.4)
!
      NUMMEMBERS=55
      NUMVOTES=399
...
!
      DO 1 I=1,NUMMEMBERS
      READ(1,100)KK,(VOTES(I+(J-1)*NUMMEMBERS),J=1,NUMVOTES)
  1   CONTINUE
!
        CALL OC(VOTES,NUMMEMBERS,NUMVOTES,DIMS,&
           POLARITY,CLASSIFY,FITS,VOLUME,&
           IDEALPOINTS,MIDPOINTS,NORMALVECTORS,EIGENVALUES,&
           EXITSTATUS)
!
!  WRITE OUT RESULTS TO CHECK PROGRAM
!
      DO 2 I=1,NUMMEMBERS
      WRITE(31,200)I,(CLASSIFY((I-1)*4+J),J=1,4),VOLUME(I),&
                  (IDEALPOINTS(I+(K-1)*NUMMEMBERS),K=1,DIMS)
  2   CONTINUE
      DO 3 I=1,NUMVOTES
      WRITE(33,200)I,(CLASSIFY(NUMMEMBERS*4+(I-1)*4+J),J=1,4),&
                   MIDPOINTS(I),&
                   (NORMALVECTORS(I+(K-1)*NUMVOTES),K=1,DIMS)
  3   CONTINUE
      WRITE(21,250)FITS(1),FITS(2)
      STOP
      END
!  &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!  &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
!
      KK=0
      DO 1 I=1,4
      DO 11 J=1,NUMVOTES+NUMMEMBERS
      KK=KK+1
      CLASSIFY(KK)=0.0
  11  CONTINUE
  1   CONTINUE
!
      DO 8990 I=1,NUMMEMBERS
      XXX(I)=0.0
 8990 CONTINUE
!
!  UN-TRANSPOSED CODE
!
      DO 1881 J=1,NUMVOTES
      NORMALVECTORS(J)=0.0
      DO 191 I=1,NUMMEMBERS
...
!
         IF(IMMM.GE.2)THEN
            ...
            ESUM=0.0
            DO 5996 I=1,500
            EIGENVALUES(I)=WVEC2(NPKADD+1-I)
            ASUM=ASUM+XMAT(I+KADD,K)
            BSUM=BSUM+ZMAT2(I,NPKADD+1-K)
            CSUM=CSUM+XMAT(I+KADD,K)*XMAT(I+KADD,K)
            DSUM=DSUM+ZMAT2(I,NPKADD+1-K)*ZMAT2(I,NPKADD+1-K)
            ESUM=ESUM+XMAT(I+KADD,K)*ZMAT2(I,NPKADD+1-K)
 5996       CONTINUE
 ...
 5995       CONTINUE
         ...
         DO 7272 I=1,NPKADD
         IF(IMMM.EQ.1)THEN
            XMAT(I+KADD,K)=ZMAT2(I,NPKADD+1-K)
         ENDIF
         IF(IMMM.GE.2)THEN
            IF(FV1(K).GT.0.0)XMAT(I+KADD,K)=ZMAT2(I,NPKADD+1-K)
            IF(FV1(K).LT.0.0)XMAT(I+KADD,K)=-ZMAT2(I,NPKADD+1-K)
         ENDIF
!
 7272    CONTINUE
 7171    CONTINUE
...
!      WRITE(23,2008)I,ISUM,JSUM
! 2008 FORMAT(8I6)
      MMSUM(I)=ISUM
  1   NNSUM(I)=JSUM
!      DO 9339 J=1,NRCALL
!      WRITE(23,2008)J,KKSUM(J),LLSUM(J)
! 9339 CONTINUE
      DO 3 I=1,NP
      LL=MMSUM(I)+NNSUM(I)
...
!      IF(IPRINT.EQ.1)WRITE(23,1002)I,LMARG(I),KMARG(I),AA
      IF(IPRINT.EQ.1)WRITE(23,1002)I,KMARG(I),AA
  5   CONTINUE
! 1002 FORMAT(I4,1X,A10,I5,F7.3)
 1002 FORMAT(I4,1X,10x,I5,F7.3)
...
!
  ...
  1   CONTINUE
      XCHK2=0.0
      DO 4 I=1,NPKADD
!      WRITE(23,4774)I,KROW(I),XROW(I)
! 4774 FORMAT(2I5,F8.3)
...
!      IF(IPRINT.EQ.1)WRITE(*,1000)XCHK1,XCHK2,XMAT
!
...
!      IF(IPRINT.EQ.1)WRITE(*,100)II,SSE1,RRSQ,AKKK,KK
...
!      IF(IPRINT.EQ.1)WRITE(*,100)II,SSE2,RRSQ,AKKK,KK
...
!         WRITE(*,8251)ITR,NS,KTT,KT,1.0-AB,AB,PRE1
      ENDIF
!      IF(IVOT.NE.NV)THEN
!         WRITE(*,330)IVOT,NV
!         STOP
...
!         WRITE(*,8252)ITR,NS,KTT,KT,1.0-AB,AB,PRE,SQRT(RR)
      ENDIF
      XCTL(ITR)=AB
!
...
!      IF(IPRINT.EQ.1)WRITE(*,102)IIJJ,KINITIAL
      MPERM(1)=0
      DO 1 I=1,N
      KPERM(I)=I
  1   CONTINUE

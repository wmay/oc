...
!      IF(IPRINT.EQ.1)WRITE(11,1005)NP,NPLOOP,NPKADD,NPREMAIN
...
!
      IF(NP.GE.1500)THEN
         ...
         CALL KPASCORE(NPMAX,NP,NPKADD,KADD,NRCALL,NS,NDUAL,11, &
...
!
      IF(NP.LT.1500)THEN
         CALL KPASCORE(NPMAX,NP,NP,KADD,NRCALL,NS,NDUAL,11,IPRINT, &
                        ZMAT2,WVEC2,DSTAR,LDATA)
      ENDIF
!
      DO 44 I=1,NP
      VOLUME(I)=0.0
      IF(NP.LT.1500)EIGENVALUES(I)=WVEC2(NP+1-I)
      DO 58 K=1,NS
      IF(NP.LT.1500)THEN
...
! **************************************************************************
!
      SUBROUTINE KPASCORE(NPMAX,NP,NPKADD,KADD,NRCALL,NS,NDUAL,KIO, &
                        IPRINT,ZMAT2,WVEC2,DSTAR,LDATA)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DIMENSION WVEC2(NPMAX),ZMAT2(NPMAX,NPMAX), &
                          DSTAR(NPMAX,NPMAX),LDATA(NP,NRCALL)
!
...
!     SCORE MATRIX
!
      CALL KPRS(NPMAX,NPKADD,YCENTER,WVEC2,1,ZMAT2,FV1,FV2,IER)
      SUM2=0.0
      DO 20 I=1,NPKADD
  ...
  20  CONTINUE
      ...
      IZULU=MIN(NPKADD,10)
      DO 10 I=1,IZULU
      XPER2=(ABS(WVEC2(NPKADD+1-I))/SUM2)*100.0
      YPER2=YPER2+XPER2
!      IF(IPRINT.EQ.1)WRITE(11,102)I,WVEC2(NPKADD+1-I),XPER2,YPER2
  10  CONTINUE
      DEALLOCATE(XCOL)
      DEALLOCATE(XROW)
      DEALLOCATE(KROW)
...
!
      ...
      DOUBLE PRECISION C,C2,DL1,EL1,F,G,H,P,R,S,TST1, &
        TST2,PYTHAG
      DOUBLE PRECISION:: S2=0.0,C3=0.0 
!
...
!
      ...
      DOUBLE PRECISION F,G,H,P,R,S,T,EPSLON,PYTHAG
      DOUBLE PRECISION:: B=0.0,C=0.0 
!
...
!
  ...
  944 CONTINUE
      ...
      IF(I.NE.J)ASUM=ASUM+ABS(SUM)
  933 CONTINUE
!      IF(IPRINT.EQ.1)WRITE(11,1091)ASUM
!
!  (X'X)-1*X'
!
      DO 85 I=1,NP
...
!         WRITE(11,825)ITR,IVOT,KT,KTT,AB,XSUM,PRE1,XLOW,XHIGH
!      ENDIF
...
!
!      IF(NS.GT.1.AND.IPRINT.EQ.1)THEN
!         WRITE(11,825)ITR,NP,KT,KTT,AB,XSUM,PRE,ALFA,BETA,RR
!      ENDIF
!      IF(NS.EQ.1.AND.IPRINT.EQ.1)THEN
!         WRITE(11,8252)ITR,NS,KTT,KT,1.0-AB,AB,PRE,SQRT(RR)
!      ENDIF
      XCTL(ITR)=AB
!
